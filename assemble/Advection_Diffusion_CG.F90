...
contains
    ...
    colour_loop: do clr = 1, size(colours)
      ...
      element_loop: do nnid = 1, len
         ...
         call assemble_advection_diffusion_element_cg(ele, t, matrix, rhs, &
              ...
              density, olddensity, pressure, heatcap, reftemp, porosity_theta, &
  ...
  subroutine assemble_advection_diffusion_element_cg(ele, t, matrix, rhs, &
                                      ...
                                      heatcap, reftemp, porosity_theta, supg_shape)
    ...
    if(have_advection.and.((equation_type==FIELD_EQUATION_INTERNALENERGY).or.(equation_type==FIELD_EQUATION_MANTLEANELASTICENERGY))) then
    ...
    ! Mass
    if(have_mass) call add_mass_element_cg(ele, test_function, t, density, olddensity, heatcap, porosity_theta, detwei, detwei_old, detwei_new, matrix_addto, rhs_addto)
    
    ! Advection
    if(have_advection) call add_advection_element_cg(ele, test_function, t, &
                                        velocity, grid_velocity, diffusivity, &
                                        density, olddensity, heatcap, &
                                        dt_t, du_t, dug_t, drho_t, dcp_t, detwei, j_mat, matrix_addto, rhs_addto)
  ...
  subroutine add_mass_element_cg(ele, test_function, t, density, olddensity, heatcap, porosity_theta, detwei, detwei_old, detwei_new, matrix_addto, rhs_addto)
    integer, intent(in) :: ele
  ...
  subroutine assemble_advection_diffusion_face_cg(face, bc_type, t, t_bc, t_bc_2, matrix, rhs, positions, velocity, grid_velocity, density, olddensity, heatcap)
    ...
    if(have_advection .and. integrate_advection_by_parts) &
      call add_advection_face_cg(face, bc_type, t, t_bc, velocity, grid_velocity, density, olddensity, heatcap, detwei, normal, matrix_addto, rhs_addto)
  ...
  subroutine add_advection_face_cg(face, bc_type, t, t_bc, velocity, grid_velocity, density, olddensity, heatcap, detwei, normal, matrix_addto, rhs_addto)
    integer, intent(in) :: face
    integer, intent(in) :: bc_type
    type(scalar_field), intent(in) :: t
    type(scalar_field), intent(in) :: t_bc
